...
R searches the current libraries (directories of installed packages). If it finds the package, it loads it into memory and makes it available for use.
...
You can explore which packages exist in each library path
...
> Add-on packages (from CRAN or under development) are usually installed in the user library
